...
2. **Секция результатов** - для отображения транскрибированного текста с возможностью:
   - Переключения между обычным текстом и текстом с временными метками
   - Копирования результата в выбранном формате
   - Скачивания файла (с суффиксом `_with_timestamps` при включенных метках)

## Особенности реализации

1. **Поддержка временных меток**: Приложение использует `verbose_json` формат для получения детальной информации о времени
2. **Обработка ошибок API**: Приложение корректно обрабатывает нестандартные ответы от API, включая нарушение формата JSON
3. **Логирование**: Подробное логирование на сервере для отладки проблем с API
4. **UI компоненты**: Использование переиспользуемых компонентов из библиотеки Shadcn/UI

## Временные метки (Timestamps)

Приложение поддерживает отображение временных меток для каждого сегмента транскрипции:

- **Формат отображения**: `[MM:SS.s - MM:SS.s] текст сегмента`
- **Переключение**: Кнопка-переключатель в интерфейсе результатов
- **Копирование/скачивание**: Поддерживает оба формата (с метками и без)
- **API конфигурация**: Использует `timestamp_granularities: "word,segment"` для получения детальной информации
